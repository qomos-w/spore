	...
	validateStep = func(step PipelineStepAST) {
		...
		if step.Invoke == "" {
			diags = append(diags, Diagnostic{
				...
				Category: "validate",
				Severity: "error",
				Message:  fmt.Sprintf("step %q in pipeline %q is missing required field 'invoke'", step.Name, pl.Name),
		...
		if !isValidInvokeRef(step.Invoke) {
			diags = append(diags, Diagnostic{
				...
				Category: "validate",
				Severity: "error",
				Message:  fmt.Sprintf("step %q in pipeline %q has invalid invoke ref %q", step.Name, pl.Name, step.Invoke),
				Hint:     "use format \"<schemaID>:<callableName>\" or \"tool:<toolName>\"",
				Line:     step.Line,
				Field:    step.Name,
				Actual:   step.Invoke,
			})
