...
// binding layer, producing a ViewProjection suitable for transport encoding.
//
// The component data (a Go struct pointer) is bound to the given ObjectDesc
// and projected via binding.ProjectView. This is the primary integration
// point between the runtime carrier and the script/schema/binding layer.
//
// Returns an error if the entity is not alive, the component is not found,
// or the binding validation fails.
//
// Contract: public semantic contract — schema-guided projection from
// runtime entity to transport-consumable view.
func (w *World) ProjectEntity(e Entity, componentName string, classDesc schema.ObjectDesc) (*binding.ViewProjection, error) {
	state := w.entityState(e)
	if state == nil || state.disposed {
		return nil, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("entity not alive")}
	}

	data, ok := w.stateComponent(state, componentName)
	if !ok {
		return nil, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("component %q not found on entity", componentName)}
	}

	b, err := binding.NewObjectBinding(classDesc, e.ID(), data)
	if err != nil {
		return nil, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("binding component %q: %w", componentName, err)}
	}

	view, err := binding.ProjectView(b)
	if err != nil {
		return nil, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("projecting component %q: %w", componentName, err)}
	}
...
func (w *World) ProjectEntityAll(e Entity, componentDescs map[string]schema.ObjectDesc) (*binding.ViewProjection, error) {
	state := w.entityState(e)
	if state == nil || state.disposed {
		return nil, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("entity not alive")}
	}
	...
	for compName, classDesc := range componentDescs {
		data, ok := w.stateComponent(state, compName)
		if !ok {
		...
		b, err := binding.NewObjectBinding(classDesc, e.ID(), data)
		if err != nil {
			return nil, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("binding component %q: %w", compName, err)}
		}

		view, err := binding.ProjectView(b)
		if err != nil {
			return nil, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("projecting component %q: %w", compName, err)}
		}
...
// from view to runtime entity.
func (w *World) PatchEntity(e Entity, componentName string, classDesc schema.ObjectDesc, view *binding.ViewProjection) ([]binding.MutationResult, error) {
	state := w.entityState(e)
	if state == nil || state.disposed {
		return nil, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("entity not alive")}
	}
	...
	b, err := binding.NewObjectBinding(classDesc, e.ID(), data)
	if err != nil {
		return nil, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("binding component %q for patch: %w", componentName, err)}
	}

	mutations, err := binding.ApplyViewPatch(b, view)
	if err != nil {
		return nil, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("patching component %q: %w", componentName, err)}
	}

	// Mark the component as changed after successful patch
	if len(mutations) > 0 {
		maskSet(&state.changed, w.compID(componentName))
	}

	return mutations, nil
}

// ProjectView projects a component of an entity through the full chain:
// runtime component → schema binding → view projection → transport encoding.
// This produces a transport.View ready for wire transmission.
//
// Contract: public semantic contract — full-chain projection from
// runtime entity to transport-encoded view.
func (w *World) ProjectEntityToTransport(e Entity, componentName string, classDesc schema.ObjectDesc, codec transport.Codec) (transport.View, error) {
	view, err := w.ProjectEntity(e, componentName, classDesc)
	if err != nil {
	...
	if err != nil {
		return transport.View{}, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("encoding component %q: %w", componentName, err)}
	}

	return tv, nil
}

// ProjectEntityChangesToTransport projects the current entity ChangeSet into a
// transport patch view. Added and changed components are schema-projected via
// the binding layer; removed components are emitted as explicit component names.
//
// Contract: public semantic contract — runtime state change projection to a
// transport patch view without making transport the change authority.
func (w *World) ProjectEntityChangesToTransport(e Entity, componentDescs map[string]schema.ObjectDesc, codec transport.Codec) (transport.View, error) {
	state := w.entityState(e)
	if state == nil || state.disposed {
		return transport.View{}, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("entity not alive")}
	}
	...
	add := func(bucketName string, componentName string, bucket map[string]any) error {
		...
		if !ok {
			return &EntityError{EntityID: e.ID(), Err: fmt.Errorf("schema descriptor for %s component %q not found", bucketName, componentName)}
		}
		view, err := w.ProjectEntity(e, componentName, classDesc)
		if err != nil {
			return err
	...
	if err != nil {
		return transport.View{}, &EntityError{EntityID: e.ID(), Err: fmt.Errorf("encoding entity changes: %w", err)}
	}
